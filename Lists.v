...
Fixpoint filter (f: nat -> bool) (l:natlist) : natlist :=
  match l with
    | [] => []
    | n :: ns =>
      if f n then n :: filter f ns else filter f ns
  end.
...
Definition posb (n : nat) : bool := negb (zerob n).

Definition nonzeros : natlist -> natlist := filter posb. 
...
Definition oddmembers : natlist -> natlist := filter oddb.
