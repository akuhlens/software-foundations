...
Proof. intros n. induction n as [| n' IHn'].
       ...
       - intros. simpl. rewrite -> IHn'. apply plus_n_Sm.
Qed.
...
Theorem plus_assoc : forall n m p : nat,
  n + (m + p) = (n + m) + p.
Proof.
  intros n m p. induction n as [| n' IHn']; simpl. 
  - reflexivity. 
...
Proof.
  intros n. induction n as [| n' IHn']; simpl.
  - reflexivity. 
...
Proof. induction n as [| n' IHn'].
       + trivial.
       + rewrite -> IHn'. rewrite negb_involutive. trivial.
Qed.
...
Proof.
  intros n m p.
  assert (L1: n + (m + p) = (n + m) + p).
  { apply plus_assoc. }
  assert (L2 : n + m = m + n).
  { apply plus_comm. }
  assert (L3 : (m + n) + p = m + (n + p)).
  { rewrite plus_assoc. reflexivity. }
  rewrite L1. rewrite L2. rewrite L3. 
  reflexivity. 
Qed.
...
Theorem mult_comm : forall m n : nat,
  m * n = n * m.
Proof.
  intros m n. induction m as [| m' IHm' ]; simpl.
  - apply mult_n_O.
  - rewrite mult_n_Sm. rewrite IHm'. reflexivity. 
Qed.
(** [] *)
...
Theorem plus_swap' : forall n m p : nat,
  n + (m + p) = m + (n + p). 
Proof with (reflexivity). intros n m p.
...
Theorem nat_to_bin_to_nat : forall n, bin_to_nat (nat_to_bin n) = n.
Proof with reflexivity.
...
T Z == T (T Z) 
IE Binaries have no normal form.
...
Example incr1 : incr Z = ST Z.
Proof. reflexivity. Qed.
Example incr2 : incr (ST Z) = T (ST Z).
Proof. simpl. reflexivity. Qed.
Example incr3 : incr (T (ST Z)) = ST (ST Z).
Proof. reflexivity. Qed.
Example incr4 : incr (ST (ST Z)) = T (T (ST Z)).
Proof. reflexivity. Qed.
Example incr5 : incr (T (T (ST Z))) = ST (T (ST Z)).
Proof. reflexivity. Qed.

Definition normalize_bin (b : bin) : bin := nat_to_bin (bin_to_nat b).

Theorem bin_to_nat_to_bin :
  forall b, nat_to_bin (bin_to_nat b) = normalize_bin b. 
Proof. reflexivity. Qed.
(** [] *)
